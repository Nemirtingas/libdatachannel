...
int main(int argc, char **argv) try {
	...
	ws->onMessage([&config, wws = make_weak_ptr(ws)](auto data) {
		...
		if (!workarounds::holds_alternative<std::string>(data))
			return;

		json message = json::parse(boost::get<std::string>(data));
	...
	while (true) {
		...
		dc->onMessage([id, wdc = make_weak_ptr(dc)](auto data) {
			// data holds either std::string or rtc::binary
			if (workarounds::holds_alternative<std::string>(data))
				std::cout << "Message from " << id << " received: " << boost::get<std::string>(data)
				          << std::endl;
			else
				std::cout << "Binary message from " << id
				          << " received, size=" << boost::get<rtc::binary>(data).size()
				          << std::endl;
		});
